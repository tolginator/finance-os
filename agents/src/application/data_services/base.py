...
class TTLCache:
    ...
    Thread-safe — uses a lock internally so callers sharing one instance
    across threads (e.g. via ``asyncio.to_thread``) do not need external
    synchronisation.  Not shared across processes — each Web API / MCP /
